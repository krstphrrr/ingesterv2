...
from projects.nri_data.nri_tools.helpers import header_fetch, extract_fields, type_lookup, dbkey_gen
    ...
    def extract_fields(self, findable_string):
        ...
        while steps<2:
            for file in os.listdir(self.path):
                ...
                if (file.find(f'{findable_string}')!=-1) and(file.find('Dump Columns')!=-1) and (file.startswith('~$')==False) and (file.endswith('.xlsx')==True) and (steps==1):
    ...
    def append_fields(self, findable_string):
        ...
        while steps<2:
            for file in os.listdir(self.path):
                ...
                if (file.find(findable_string)!=-1) and (file.endswith('.xlsx')==False) and (steps==1):
                    for item in os.listdir(os.path.join(self.path, file)):
